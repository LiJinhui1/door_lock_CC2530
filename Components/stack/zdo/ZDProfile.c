...
#define ZADDR_TO_AFADDR( pZADDR, AFADDR ) {                            \
  ...
  osal_memcpy((AFADDR).addr.extAddr, (pZADDR)->addr.extAddr, Z_EXTADDR_LEN); \
}
...
CONST zdpMsgProcItem_t zdpMsgProcs[] =
{
  ...
  { Mgmt_NWK_Disc_rsp,      AT_ZDO_ProcessMgmtNwkDiscRsp},
#endif
 ...
 *
 * @param       nodeId - interest of short address
 ...
 * @return      afStatus_t
 */
afStatus_t ZDP_IEEEAddrReq( uint16 shortAddr, uint16 nodeId, byte ReqType,
                            byte StartIndex, byte SecurityEnable )
{
  uint8 *pBuf = ZDP_TmpBuf;
  ...
  *pBuf++ = LO_UINT16( nodeId );
  *pBuf++ = HI_UINT16( nodeId );
                            ...
                            byte SecurityEnable )
{
  ...
  len += (NetworkListCount * ( ZDP_NETWORK_EXTENDED_DISCRIPTOR_SIZE ));
  if ( notdoSCANCmd ) { // AT+SCAN:1 Cmd's rsp need PANID
    len -= NetworkListCount * 2;
  }
