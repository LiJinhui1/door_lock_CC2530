...
static uint8 bdb_repAttrCfgRecordsArrayAdd(uint8 endpoint, uint16 cluster, uint16 attrID, uint16 minReportInt, uint16 maxReportInt, uint8 reportableChange[], uint16 defMinReportInt, uint16 defMaxReportInt, uint8 defReportChange[]);
...
static uint16 bdb_RepCalculateEventElapsedTime(uint32 remainingTimeoutTimer, uint16 nextEventTimeout)
{
	...
	uint32 nextEventTimeout_32 = (uint32)nextEventTimeout;

	passTimeOfEvent = nextEventTimeout_32 * 1000 >= remainingTimeoutTimer ? nextEventTimeout_32 * 1000 - remainingTimeoutTimer : 0;
...
void bdb_RepUpdateMarkBindings(void)
{
	...
	for (i = 0; i < bdb_reportingClusterEndpointArrayCount; i++) {
		...
		if (bEntry != NULL) { //Found a binding with the given cluster and endpoint, mark the Endpoint-cluster entry (this activates reporting)
...
static uint8 bdb_clusterEndpointArrayGetMin(void)
{
	...
	for (i = 0; i < bdb_reportingClusterEndpointArrayCount; i++) {
		if (FLAGS_CHECKFLAG(bdb_reportingClusterEndpointArray[i].flags, BDBREPORTING_HASBINDING_FLAG_MASK) == BDBREPORTING_TRUE) { //Only do with valid entries (HasBinding==true)
