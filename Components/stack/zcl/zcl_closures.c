...
static ZStatus_t zclClosures_ProcessInDoorLockClearAllRFIDCodes( zclIncoming_t *pInMsg,
                                                                 zclClosures_DoorLockAppCallbacks_t *pCBs );
static ZStatus_t zclClosures_ProcessInDoorLockSetTemporaryPin( zclIncoming_t *pInMsg,
                                                               zclClosures_DoorLockAppCallbacks_t *pCBs );
static ZStatus_t zclClosures_ModifyPin( zclIncoming_t *pInMsg,
                                        zclClosures_DoorLockAppCallbacks_t *pCBs );
                                                    ...
                                                    zclClosures_DoorLockAppCallbacks_t *pCBs )
{
  ...
  if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
  {
    switch(pInMsg->hdr.commandID)
    {
      ...
      case COMMAND_CLOSURES_SET_TEMPORARY_PIN:
        stat = zclClosures_ProcessInDoorLockSetTemporaryPin( pInMsg, pCBs );
        break;
        
      case COMMAND_CLOSURES_MODIFY_PIN:
        stat = zclClosures_ModifyPin( pInMsg, pCBs );
        break;            
...
/*********************************************************************
 * @fn      zclClosures_ProcessInDoorLockSetTemporaryPin
 *
 * @brief   Process in the received Set Temporary Pin cmd
 *
 * @param   pInMsg - pointer to the incoming message
 * @param   pCBs - pointer to the application callbacks
 *
                                                               ...
                                                               zclClosures_DoorLockAppCallbacks_t *pCBs )
{
  ...
  if ( pCBs->pfnDoorLockSetTemporaryPin )
  {
    zclDoorLockSetTemporaryPin_t cmd;

    // adminPin
    j = offset;
    cmd.adminPin = (uint8 *)zcl_mem_alloc( pInMsg->pData[j] + 1 );
    if ( !cmd.adminPin )
    {
      return ( ZMemError );  // no memory
    }
    for ( i=0; i<pInMsg->pData[j]+1; i++ )
    {
      cmd.adminPin[i] = pInMsg->pData[offset++];
    }

    // tempPin
    j = offset;
    cmd.tempPin = (uint8 *)zcl_mem_alloc( pInMsg->pData[j] + 1 );
    if ( !cmd.tempPin )
    {
      zcl_mem_free( cmd.adminPin );
      return ( ZMemError );  // no memory
    }
    for ( i=0; i<pInMsg->pData[j]+1; i++ )
    {
      cmd.tempPin[i] = pInMsg->pData[offset++];
    }

    // activeTimes
    cmd.activeTimes = pInMsg->pData[offset++];

    // activeTime
    cmd.activeTime = BUILD_UINT32( pInMsg->pData[offset], pInMsg->pData[offset+1], pInMsg->pData[offset+2], pInMsg->pData[offset+3] );

    status = ( pCBs->pfnDoorLockSetTemporaryPin( pInMsg, &cmd ) );
    zcl_mem_free( cmd.tempPin );
    zcl_mem_free( cmd.adminPin );
    return status;
  }
...
/*********************************************************************
 * @fn      zclClosures_ModifyPin
 *
 * @brief   Process in the received Modify Pin cmd
 *
 * @param   pInMsg - pointer to the incoming message
 * @param   pCBs - pointer to the application callbacks
 *
...
static ZStatus_t zclClosures_ModifyPin( zclIncoming_t *pInMsg,
                                            zclClosures_DoorLockAppCallbacks_t *pCBs )
{
  if ( pCBs->pfnDoorLockModifyPin )
  {
    zclDoorLockModifyPin_t cmd;
    uint8 count,num = 0;
    
    for(count=0;count<PIN_LEN_MAX;count++)
    {
      cmd.OriginalPin[count] = pInMsg->pData[num++];
    }
    
    for(count=0;count<PIN_LEN_MAX;count++)
    {
      cmd.ModifyPin[count] = pInMsg->pData[num++];
    }
    
    return ( pCBs->pfnDoorLockModifyPin( pInMsg, &cmd ) );
  }
...
ZStatus_t zclClosures_SendSetTemporaryPinStatusResponse( uint8 srcEP, afAddrType_t *dstAddr,
                                                  uint8 status, uint8 disableDefaultRsp, uint8 seqNum )
{
  uint8 payload[1];   // 1 byte payload
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_CLUSTER_ID_CLOSURES_DOOR_LOCK,
                          COMMAND_CLOSURES_SET_TEMPORARY_PIN_RSP, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR,
                          disableDefaultRsp, 0, seqNum, sizeof( payload ), payload );
}

ZStatus_t zclClosures_SendModifyPinStatusResponse( uint8 srcEP, afAddrType_t *dstAddr,
                                                  uint8 status, uint8 disableDefaultRsp, uint8 seqNum )
{
  uint8 payload[1];   // 1 byte payload
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_CLUSTER_ID_CLOSURES_DOOR_LOCK,
                          COMMAND_CLOSURES_MODIFY_PIN_RSP, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR,
                          disableDefaultRsp, 0, seqNum, sizeof( payload ), payload );
}
